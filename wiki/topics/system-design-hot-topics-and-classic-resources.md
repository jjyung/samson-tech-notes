...
## 2026-05-04 的補充：cloud-native correctness 與 secret boundary

- `KAL – The Kube API Linter`
  - 代表 platform engineering 仍然在意 API correctness 與可驗證性。
- `Kloak: Kernel-space secret injection via eBPF on Kubernetes`
  - 代表 secret handling / injection 仍然是 K8s 與 backend 安全設計的核心邊界問題。

這兩個訊號都不算最熱，但它們很適合補強 system design 的長期視角：

- correctness 要靠工具化
- security boundary 要靠分層與明確責任
- cloud-native 的老問題並沒有因為 agent 熱潮而消失
